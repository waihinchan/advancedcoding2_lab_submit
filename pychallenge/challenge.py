...
object_char = "g fmnc wms bgblr rpylqjyrc gr zw fylb. rfyrq ufyr amknsrcpq ypc dmp. bmgle gr gl zw fylb gq glcddgagclr ylb rfyr'q ufw rfgq rcvr gq qm jmle. sqgle qrpgle.kyicrpylq() gq pcamkkclbcb. lmu ynnjw ml rfc spj."
translate = str.maketrans(input_char,output_char)
print (object_char.translate(translate))
# PythonChallenge2
import string

text = """the source long text"""
for c in text:
    ...
    for j in range(3, c-3):
        ...
        if m in string.ascii_lowercase:
            for l in range(j - 3, j + 4):
                if lines[i][l] in string.ascii_uppercase:
                    count = count + 1
                if j - 4 >= 0:
                    if lines[i][j-4] in string.ascii_uppercase:
                        count = count + 1
                if j + 4 < c:
                    if lines[i][j+4] in string.ascii_uppercase:
                        count = count + 1
